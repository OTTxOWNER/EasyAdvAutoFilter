...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("", url="t.me/TryToLiveAlon")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('↼ Eᴀʀɴ Mᴏɴᴇʏ', url=f'https://t.me/OTTxSerises'),
        ...
        buttons = [[
            InlineKeyboardButton('✉️ ᴀɴʏ ǫᴜᴇʀʏ ✉️', url='t.me/OTTxOwner')
        ], [
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            dlt = await message.reply_sticker('CAACAgIAAxkBAAECO5ll6Lxou8heD9dMXxLTCJqRojjOqgACcQgAAoSUQUlvaAkaprvOczQE', reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
